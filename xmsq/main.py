...
class XiaomiSign:
    ...
    def like_post(self):
        for _ in range(2):
            like_url = 'https://api.vip.miui.com/mtop/planet/vip/user/checkin?miui_vip_ph=QcEauXLOuB6NAKE6GV%2FE5g%3D%3D'
            headers = {
                'Host': 'api.vip.miui.com',
                'Connection': 'keep-alive',
                'User-Agent': 'Mozilla/5.0 (Linux; U; Android 10; zh-cn; M2007J1SC Build/RKQ1.200826.002) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/89.0.4389.116 Mobile Safari/537.36 XiaoMi/MiuiBrowser/15.7.22 app/vipaccount',
            ...
            response = requests.post(url=like_url, headers=headers)
...
if __name__ == '__main__':
    print('🔔小米社区 | 开始 | ' + time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
    #检测账户变量
